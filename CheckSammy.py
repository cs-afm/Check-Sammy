...
import os,hashlib,json,threading,datetime
...
class SammyGUI(tk.Tk):
    ...
    def __init__(self):
        ...
        self.check_this = {'F':[],'D':[]}
        self.checked = {'Ok':[],'Corrupted':[],'No md5':[],'Missing file':[],'New file':[]}

        self.button_frame = tk.Frame(self)
        self.button_frame.grid(row=1,column=0,pady=5,padx=5)

        self.recursive_button = tk.Button(self.button_frame,text='   All in one folder   ',command=self.sel_pathr)
        self.recursive_button.grid(row=1,column=0,sticky='N',padx=2,pady=10)

        self.folder_button = tk.Button(self.button_frame,text='Single folder',command=self.sel_dir)
        self.folder_button.grid(row=2,column=0,sticky='N',padx=2,pady=2)

        self.file_button = tk.Button(self.button_frame,text='Single file',command=self.sel_file)
        self.file_button.grid(row=3,column=0,sticky='N',pady=2)

        self.start_button = tk.Button(self.button_frame,text='Start',command=lambda: threading.Thread(target=self.start,args=()).start())
        ...
        self.batch_yscrollbar = tk.Scrollbar(self.batch_frame,orient='vertical')
        self.batch_yscrollbar.grid(row=1,column=2,rowspan=2,sticky='NS')

        self.batch_xscrollbar = tk.Scrollbar(self.batch_frame,orient='horizontal')
        self.batch_xscrollbar.grid(row=2,column=0,columnspan=2,sticky='WE')

        self.batch_label = tk.Label(self.batch_frame,text='Queue (0)',borderwidth=4,relief='groove',width=100)
        self.batch_label.grid(row=0,column=1,sticky='W',pady=5)

        self.batch_button = tk.Button(self.batch_frame,text='Delete',command=self.delete_item)
        self.batch_button.grid(row=0,column=0,sticky='W',pady=5,padx=10)

        self.batch_listbox = tk.Listbox(self.batch_frame,yscrollcommand=self.batch_yscrollbar.set,xscrollcommand=self.batch_xscrollbar.set,width=180,height=23)
        ...
        self.batch_frame.bind('<Leave>',lambda x: self.batch_listbox.selection_clear(0,END))
        ...
        self.calculate = tk.Radiobutton(self.control_frame,text='Calculate md5',variable=self.sel_operation,value=1,tristatevalue=4,command=lambda: self.status_label.configure(text='Ready'))
        self.calculate.grid(row=1,column=0,sticky='w')
        self.difference = tk.Radiobutton(self.control_frame,text='Check difference',variable=self.sel_operation,value=2,tristatevalue=4,command=lambda: self.status_label.configure(text='Ready'))
    ...
    def delete_item(self):
        ...
        try:
            item_text = self.batch_listbox.get(self.batch_listbox.curselection())
            if item_text[2] == 'F':
            ...
            self.batch_label.configure(text='Queue (%s)' % str(self.batch_listbox.size()))
        except:
            pass
    ...
    def sel_pathr(self):
        ...
        if dir != '':
            ...
            for d in dirs:
                self.check_this['D'].append(os.path.abspath(os.path.join(root,d)))
            for f in files:
                if  not '.md5' in f:
                    self.check_this['F'].append(os.path.abspath(os.path.join(root,f)))
            self.update_batch()
    ...
    def report(self):
        ...
        if no_cs > 0:
            ...
            self.report_text.insert(END,'All the .md5 were found.\n------------\n\n\n')

        if corr > 0:
            self.report_text.insert(END,'The following files/folders are corrupted:\n\n')
            for obj in self.checked['Corrupted']:
                self.report_text.insert(END,"'"+obj+"'\n")
            self.report_text.insert(END,'---\n\n\n')

        if missing > 0:
            self.report_text.insert(END,'The following files/folders are missing:\n\n')
            for obj in self.checked['Missing file']:
                self.report_text.insert(END,"'"+obj+"'\n")
            self.report_text.insert(END,'---\n\n\n')

        if new > 0:
            self.report_text.insert(END,'The following files/folders are new/unknown:\n\n')
            for obj in self.checked['New file']:
                self.report_text.insert(END,"'"+obj+"'\n")
            self.report_text.insert(END,'---\n\n\n')

        if no_cs > 0:
            self.report_text.insert(END,'The following .md5 files are missing:\n\n')
            for obj in self.checked['No md5']:
                self.report_text.insert(END,"'"+obj+"'\n")
            self.report_text.insert(END,'---\n\n\n')

        if ok > 0:
            self.report_text.insert(END,'The following files/folders are ok:\n\n')
            for obj in self.checked['Ok']:
                self.report_text.insert(END,"'"+obj+"'\n")
            self.report_text.insert(END,'---\n\n\n')

        self.report_text.insert(END,'Report generated by \'Check Sammy %s\' on %s' % (self.version, str(datetime.date.today())))
            ...
            try:
                ...
                for root,folders,files in os.walk(ff):
                    for file in files:
                        ...
                        new_hash_dict[to_hash.replace(ff,'')[1:]] = self.calc_md5(to_hash)
